...
A simple hello world agent built with Google ADK which is non-A2A compatible.
...
```bash
uvicorn main:root_agent --host localhost --port 8001
```
...
```bash
docker run -e GOOGLE_API_KEY=your-api-key -p 8001:8001 hello-world-agent
```
